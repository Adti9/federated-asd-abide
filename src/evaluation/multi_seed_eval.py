...
def run_multi_seed_benchmark():
    ...
    for seed in seeds:
        train_df, test_df = train_test_split(
            filtered_df, test_size=0.20, random_state=seed, stratify=filtered_df['target']
        )

        X_tr, y_tr = transform_features(train_df, cat_features)
        X_te, y_te = transform_features(test_df, cat_features)
        ...
        res_iid = sim_iid.fit_evaluate(iid_splits, test_df, cat_features)
        ...
        res_non_iid = sim_non_iid.fit_evaluate(site_splits, test_df, cat_features)
    ...
    print("\n=== FINAL BENCHMARK (DETERMINISTIC LEAKAGE-FREE FEATURE SPACES) ===")
    summary_data = []
    for setting, metrics in results.items():
        ...
        print(f"{setting:12s} | ROC-AUC: {auc_mean:.4f} ± {auc_std:.4f} | ACC: {acc_mean:.4f} ± {acc_std:.4f} | F1: {f1_mean:.4f} ± {f1_std:.4f}")
        summary_data.append({
            'Setting': setting,
